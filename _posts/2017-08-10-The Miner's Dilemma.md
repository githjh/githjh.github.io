...
Lemma 1 (수익 수렴). 만약 침투 비율이 일정하다면, 풀의 수익은 수렴한다.

증명. 스탭 t에서 풀 i의 수익 밀도를 r_i(t)라고 하자. 

우리는 r(t) = (r_1(t), ...,r_p(t))^T 로 이익 밀도 백터를 정의한다.
...
풀 지식 : 단계를 수행하는 풀은 침입자의 공격 속도 (자신의 신원이 아님)와 다른 풀의 수익률을 알고 있다. 이 지식은 섹션 IV-E에서 설명했듯이 풀의 수익을 최적화하는 데 필요하다. 

풀은 II-C 장에서 설명한 바와 같이 광부로부터받는 업무의 부분 및 전체 증명 율을 비교하여 공격당한 비율을 추정 할 수 있습니다. 다른 풀 각각에 대한 공격 비율을 계산하기 위해 풀은 두 가지 방법 중 하나를 사용할 수 있다. 첫째, 풀은 종종이 데이터를 게시하여 광부에게 정직함을 입증한다 [23, 15, 12]. 둘째, 풀은 일부 다른 탐사 광력으로 다른 풀에 잠입하여 풀에서 프로브의 보상을 직접 모니터링하여 수익 밀도를 직접 측정 할 수 있다.

#### E. 일반적인 분석

mi는 수영장에 충성하는 광부의 수 이다. xi, j (t)는 단계 t에서 풀 j에 잠입하기 위해 풀 i가 사용하는 광부의 수 이다. 

따라서 풀 i의 채광률은 충성하고 있는 마이너의 숫자에서 침입을 위해 사용하는 마이너를 뺀 숫자 이다. 이 절대 광업 율은 시스템의 총 광산 비율, 즉 BWH 공격에 관여하지 않는 모든 광부의 수로 나뉩니다. 이 단계 t에서의 마이닝 비율은 아래와 같다.

t 단계가 끝날 때 풀 i의 수익 밀도는 직접 광산에서 발생한 수익과 침투 된 수영장에서 얻은 수익을 충성도가 높은 광부 수와 함께 블록 원천 징수 침입자로 나눈 값입니다.

### V. 공격자가 한명인 경우

  우리는 풀 1이 풀 2에 침투 할 수 있지만 풀 2에서는 풀 1으로는 침입을 할 수 없는 간단한 게임으로 분석을 시작한다. 솔로 마이너와 풀 1 풀 2 의 수가 m-m1-m2 있다고 하자.(솔로 마이너는 공격하지 않고 공격 할 수 없다). 이 시나리오는 그림 3에 설명되어 있다. 점선으로 표시된 빨간색 화살표는 풀 1의 채굴 능력 x1,2가 풀 BWH 공격으로 풀 2에 침투하고 있음을 나타낸다. 풀 2는 공격하지 않으므로 모든 m2 충성 광부를 대신하여 일한다. 반면 풀 1은 충성 광부의 x1,2를 사용하지 않으며, 직접적인 채굴 능력은 m1-x1,2이다. 비트 코인 (Bitcoin) 시스템은 전체 증거를 발표하는 총 광부, 즉 모든 광부 (x1,2)를 공표하는 광부의 총 수를 기준으로 이러한 비율을 표준화 한다. 따라서 풀의 직접 수입은 아래와 같다.
